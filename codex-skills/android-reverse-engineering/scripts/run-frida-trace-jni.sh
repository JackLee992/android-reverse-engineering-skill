...
if [[ $# -eq 0 || "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'EOF'
Usage: run-frida-trace-jni.sh --package <package> [OPTIONS]

Trace JNI RegisterNatives activity with Frida.
...
Examples:
  run-frida-trace-jni.sh --package com.example.app
  run-frida-trace-jni.sh --package com.example.app --output output/jni-trace.jsonl
EOF
  exit 0
fi
...
exec "$FRIDA_PYTHON" "$SCRIPT_DIR/frida_trace_host.py" \
  --script "$SCRIPT_DIR/frida_trace_jni.js" \
  "$@"
